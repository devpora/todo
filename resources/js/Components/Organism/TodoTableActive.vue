...
const applyFilter = () => {
  ...
  axios
    .get(`/todo-active?page=${currentPage.value}&${queryString}`)
    .then((response) => {
      todos.value = response.data
    })
    .catch((error) => {
...
<template>
        ...
        <FwbTableRow>
          ...
          <FwbTableCell>
            <span
              ...
              @click="copyLink(todo.isPublic, todo.sharedLink)"
            >
            ...
            <div v-if="todo.isShared && !todo.isPublic">
              <div class="flex -space-x-4">
                <FwbAvatar
                  ...
                  rounded
                  stacked
                  class="rounded-full border-2 border-green-400"
                />
  ...
  <div class="flex justify-center pt-6">
    <FwbPagination
      v-if="todos.meta && todos.meta.total > 5"
      v-model="currentPage"
      :total-items="todos.meta.total"
      :per-page="todos.meta.per_page"
    ></FwbPagination>
  </div>
</template>
